...
function onChangeOption(d) {
  ...
  // 创建凹凸几何体
  var convexGeo = new ConvexGeometry(arr);
  let sss = [];
  // 根据顶点点位逆转为lab
  ...
  const result = [];
  // 每3个拆成二维数组，取出每个坐标对应的lab值
  for (let i = 0; i < sss.length; i += size) {
    result.push(sss.slice(i, i + size));
  }
  let colors = [];
  // 每个lab值转换成着色器需要的格式
  result.forEach((i) => {
    ...
    colors = colors.concat(colorRgb(c));
  });
  ...
  var cube = new THREE.Mesh(convexGeo, material);
  scene.add(cube);
}
